   ...
   implements IBundleVisitable
{
   ...
   public void accept(IBundleVisitor visitor, Object passAlongArgument)
   {
      for (Iterator iter = entries.values().iterator(); iter.hasNext();) {
         visitor.visitBundleEntry((BundleEntry)iter.next(), passAlongArgument);
      }
    ...
    * 
    * @param oldKey
    *        the bundle entry key to rename
    * @param newKey
    *        the new name for the bundle entry
    */
   protected void renameKey(String oldKey, String newKey)
   {
      BundleEntry oldEntry = (BundleEntry)entries.get(oldKey);
      if (oldEntry != null) {
         BundleEntry newEntry = new BundleEntry(newKey, oldEntry.getValue(),
                                                oldEntry.getComment());
    ...
    * 
    * @param key
    *        key of bundle entry to be commented
    */
    ...
    * 
    * @param key
    *        key of bundle entry to be uncommented
    */
   protected void uncommentKey(String key)
   {
      BundleEntry entry = (BundleEntry)entries.get(key);
      if (entry != null) {
         BundleEntry newEntry = new BundleEntry(key, entry.getValue(), entry.getComment(),
                                                false);
         addEntry(newEntry);
      }
   }
    ...
    * 
    * @param origKey
    *        key of bundle entry to be copied
    * @param newKey
    *        key for the copied bundle entry
    */
   protected void copyKey(String origKey, String newKey)
   {
      BundleEntry origEntry = (BundleEntry)entries.get(origKey);
      if (origEntry != null) {
         BundleEntry newEntry = new BundleEntry(newKey, origEntry.getValue(),
